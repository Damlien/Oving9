...
The purpose of this task is to build a controlled number generator that repeatedly produces the sequence 1, 2, 3, 4, 5, 6. This is a prerequisite block for later tasks, but D-100.16 itself is about making and testing the generator, not about displaying dice patterns.

The generator is controlled by SW1. While SW1 is held down, the circuit advances through the sequence. When SW1 is released, the generator stops and keeps the current number. Since the Go Board clock is too fast to inspect directly, the design uses an internal delay counter so each number remains visible long enough to test.

The standalone D-100.16 script shows the generated number on one 7-segment display. This makes it possible to verify the required behavior directly: the display should cycle through 1 to 6 while SW1 is held, then freeze when SW1 is released.
...
- `SW1` is passed through `debouncer`, producing `clean_SW1`.
- When `clean_SW1=1`, an internal clock-cycle counter runs. When it reaches `12_000_000`, the state `D_now` advances to the next number.
- When `clean_SW1=0`, the counter stops and `D_now` holds its current value.
- The sequence is `001 → 010 → 011 → 100 → 101 → 110 → 001`, representing decimal 1 through 6.
- `D_now` is connected to `seven_segment_display_0_F` with `D3` hardwired to `0`, so the generated values can be tested on the Go Board 7-segment display.

`lib_modules/Repeated_sequentially_counter_1_6.v` is the stripped reusable version of the same generator behavior:
...
- It does not contain the physical `SW1` input, the debouncer, or the 7-segment display wiring.
- This reusable module is later used by the D-100.17/D-100.18 top level.
